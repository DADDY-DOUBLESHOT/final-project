...
const UploadBook = ({ navigation }) => {
  ...
  const handleImagePicker = async () => {
    ...
    if (!result.canceled) {
      ...
      const uriComponents = selectedImage.uri.split("/");
      const fileName = uriComponents[uriComponents.length - 1];
      selectedImage.fileName = fileName;

      console.log("Selected image:", selectedImage);
      console.log("File name:", selectedImage.fileName);
    ...
    //     console.log("coverimage----->",result.assets[0].fileName);
  ...
  const handleUpload = async () => {
    const body = {
      ...
      pdfUrl: files.bookPdf,
      coverImg: files.coverImage,
      country: "India",
    };
    ...
    try {
      await axios(config)
        .then(function (response) {
          ...
          ToastAndroid.show(`Book Uploaded`, ToastAndroid.SHORT);
        })
        .catch(function (error) {
          console.log(formData);
          console.log(coverimage.uri);
          console.log("Unable to upload Book1", error);
          ToastAndroid.show(`Error in Book Uploaded ${error.message}`, ToastAndroid.SHORT);
        });
    } catch (error) {
  ...
  const handleUploadPdf = () => {
    ...
    try {
      ...
      axios
        .post(`${BASE_URL}admin/book/upload`, formData, config)
        .then(function (response) {
          setFiles({
          ...
          ToastAndroid.show(`Image and Pdf  Uploaded`, ToastAndroid.SHORT);
        })
        .catch(function (error) {
  ...
  return (
    <ScrollView>
      <View style={styles.container}>
            ...
            {genre_data.map((item) => {
              return (
                <View key={item.id}>
                  ...
                  >
                    <Text style={getLabelStyle(`${item.title}`)} key={item.id}>
                      {item.title}
                    </Text>
                  </TouchableOpacity>
                </View>
        ...
        <View>
          <Text style={styles.uploadbookcover} onPress={toggleModal}>
            Upload Book Cover
          </Text>
          <View style={{ flexDirection: "row", marginTop: 3 }}>
            {coverimage && (
              <>
                <Image source={{ uri: coverimage.uri }} style={styles.image} />
        ...
        <View style={styles.titleContainer}>
          <Text style={styles.uploadbook} onPress={handleDocumentPicker}>
            Upload Book
          </Text>
